...
export class BaseRepository<T, M extends Model> {
  ...
  async findById(team: string, relations: string[] = []): Promise<M> {
    if (!team) {
    ...
    return await query.findById(team);
  }

  async updateById(team: string, data: Partial<M>, trx?: Transaction): Promise<M> {
    return await this.model.query(trx).patchAndFetchById(team, data).returning("*");
  }
